  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 relative">
        ...
        <h3 className="text-lg font-bold text-property-navy mb-2">{t.title}</h3>
        <p className="text-sm text-property-text-muted mb-4">{t.description}</p>

        {/* Code block */}
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 mb-4">
          <code className="text-xs text-slate-700 break-all block whitespace-pre-wrap">
            {embedCode}
          </code>
        </div>
        ...
        <button
          onClick={handleCopy}
          className="w-full bg-property-navy text-white font-medium py-2.5 rounded-lg hover:bg-property-navy/90 transition-colors text-sm"
        >
